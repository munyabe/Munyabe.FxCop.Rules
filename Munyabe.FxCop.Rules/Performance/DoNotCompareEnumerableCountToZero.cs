...
namespace Munyabe.FxCop.Performance
{
    ...
    public class DoNotCompareEnumerableCountToZero : BaseRule
    {
        ...
        public override void VisitExpression(Expression expression)
        {
            ...
            if (binary != null)
            {
                var operand1 = binary.Operand1 as BinaryExpression;
                if (operand1 == null)
                {
                    var isEqual = binary.NodeType == NodeType.Ceq;
                    var isLessThan = binary.NodeType == NodeType.Clt;
                    var isGreaterThan = binary.NodeType == NodeType.Cgt;

                    if (isEqual || isLessThan || isGreaterThan)
                    {
                        if (IsCountAndZeroOperation(binary))
                        {
                            Problems.Add(CreateProblem(expression));
                        }
                    }
                }
                else
